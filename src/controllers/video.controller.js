...
const getAllVideos = asyncHandler(async (req, res) => {
    ...
    const paginatedPipeline = [...pipeline, { $skip: skip }, { $limit: parseInt(limit, 10) }];

    const [videos, countResult] = await Promise.all([
        Video.aggregate(paginatedPipeline),
...
const getVideoById = asyncHandler(async (req, res) => {
    ...
    // add to user's watch history regardless
    ...
    // now run aggregation to get detailed video info
    const video = await Video.aggregate([
        {
            $match: {
                _id: new mongoose.Types.ObjectId(videoId)
